...
# This microservice receives raw CV text and returns structured data using enhanced NLP with LLM.
...
OPENROUTER_MODEL = 'deepseek/deepseek-r1'  # Advanced reasoning model for complex CV analysis
...
def extract_with_openrouter_llm(cv_text):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if json_match:
                extracted_data = pyjson.loads(json_match.group(0))
                enhanced_data = enhance_with_rag(validate_and_clean_data(extracted_data))
...
def parse_cv():
    """Enhanced CV parsing with LLM and regex fallback"""
    try:
        ...
        try:
            ...
            if extracted_data:
                # Enhance with regex fallback if needed
                fallback_data = extract_with_regex_fallback(text)
                enhanced_data = merge_extraction_results(extracted_data, fallback_data)
                
                logger.info("Successfully extracted CV data using Groq LLM")
        ...
        try:
            ...
            if extracted_data:
                ...
                logger.info("Successfully extracted CV data using OpenRouter LLM")
                return jsonify(enhanced_data)
...
def merge_extraction_results(llm_data, fallback_data):
    """Merge LLM extraction with regex fallback data"""
    merged = llm_data.copy()
    
    # Use fallback data if LLM data is empty
    for key in ['name', 'email', 'phone']:
        if not merged.get(key) and fallback_data.get(key):
            merged[key] = fallback_data[key]
    
    # Merge skills
    llm_skills = set(merged.get('skills', []))
    fallback_skills = set(fallback_data.get('skills', []))
    merged['skills'] = list(llm_skills.union(fallback_skills))
